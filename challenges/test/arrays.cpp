...
TEST(Has_All_Unique2, EmptyString)
{
  EXPECT_TRUE(chl::HasAllUnique2(""));
}
...
TEST(Has_All_Unique2, NotUnique)
{
  EXPECT_FALSE(chl::HasAllUnique2("2dfg$Sp[d"));
}

TEST(IsPermutation, DifferentSizes)
{
  EXPECT_FALSE(chl::IsPermutation("wer", "wert"));
}
TEST(IsPermutation, Identical)
{
  EXPECT_FALSE(chl::IsPermutation("left", "left"));
}
TEST(IsPermutation, OneEmpty)
{
  EXPECT_FALSE(chl::IsPermutation("", "left"));
}
TEST(IsPermutation, BothEmpty)
{
  EXPECT_FALSE(chl::IsPermutation("", ""));
}
TEST(IsPermutation, Permutation)
{
  EXPECT_TRUE(chl::IsPermutation("felt", "left"));
}
TEST(IsPermutation, NotPermutation)
{
  EXPECT_FALSE(chl::IsPermutation("lift", "tiff"));
}
TEST(IsPermutation, NotPermutationNewChar)
{
  EXPECT_FALSE(chl::IsPermutation("lift", "shit"));
}
...
TEST(BackspaceCompare, Test1)
{
  EXPECT_TRUE(chl::BackspaceCompare("ab#c", "ad#c"));
}
TEST(BackspaceCompare, Test2)
{
  EXPECT_TRUE(chl::BackspaceCompare("ab##", "c#d#"));
}
TEST(BackspaceCompare, Test3)
{
  EXPECT_TRUE(chl::BackspaceCompare("a##c", "#a#c"));
}
TEST(BackspaceCompare, Test4)
{
  EXPECT_FALSE(chl::BackspaceCompare("a#c", "b"));
}
